...
RSpec.describe User, '#acceptable_image' do
  it 'adds errors if image is too big' do
    ...
    post.post_image.attach(io: File.open(Rails.root + 'spec/fixtures/too_big.png'), filename: 'too_big.png')
  ...
  it 'adds errors if image is wrong type' do
    user = create(:user, first_name: 'Joe', last_name: 'Picket')
    post = create(:post, user: user)
    post.post_image.attach(io: File.open(Rails.root + 'spec/fixtures/wrong_type.fig'), filename: 'wrong_type.fig')

    expect(post.errors.full_messages).to include('Post image must be a JPEG or PNG')
  end
end
